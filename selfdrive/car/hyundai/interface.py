...
class CarInterface(CarInterfaceBase):
    ...
    def _get_params(ret, candidate, fingerprint, car_fw, experimental_long):
        ret.carName = "hyundai"
        ret.radarUnavailable = RADAR_START_ADDR not in fingerprint[
            1] or DBC[ret.carFingerprint]["radar"] is None
        ...
        if candidate in (CAR.SANTA_FE, CAR.SANTA_FE_2022, CAR.SANTA_FE_HEV_2022, CAR.SANTA_FE_PHEV_2022):
            ...
            ret.mass = {CAR.KONA_EV: 1685., CAR.KONA_HEV: 1425.,
                        CAR.KONA_EV_2022: 1743.}.get(candidate, 1275.) + STD_CARGO_KG
            ret.wheelbase = 2.6
            ...
            # weight from Limited trim - the only supported trim
        ...
        # Kia
            ...
            # weight from SX and above trims, average of FWD and AWD versions
        ...
        if candidate in CANFD_CAR:
            ...
            ret.experimentalLongitudinalAvailable = candidate in (
                HYBRID_CAR | EV_CAR) and candidate not in CANFD_RADAR_SCC_CAR
        else:
            ret.longitudinalTuning.kpV = [0.5]
            ret.longitudinalTuning.kiV = [0.0]
            ret.experimentalLongitudinalAvailable = candidate not in (
                LEGACY_SAFETY_MODE_CAR | CAMERA_SCC_CAR)
        ...
        if candidate in CANFD_CAR:
            ...
            if candidate in LEGACY_SAFETY_MODE_CAR:
                ...
                ret.safetyConfigs = [get_safety_config(
                    car.CarParams.SafetyModel.hyundai, 0)]
    ...
    def init(CP, logcan, sendcan):
        ...
        if CP.flags & HyundaiFlags.ENABLE_BLINKERS:
            disable_ecu(logcan, sendcan, bus=5, addr=0x7B1,
                        com_cont_req=b'\x28\x83\x01')
    ...
    def _update(self, c):
        ...
        if self.CS.cruise_buttons[-1] != self.CS.prev_cruise_buttons:
            buttonEvents.append(create_button_event(
                self.CS.cruise_buttons[-1], self.CS.prev_cruise_buttons, BUTTONS_DICT))
            # Handle CF_Clu_CruiseSwState changing buttons mid-press
            if self.CS.cruise_buttons[-1] != 0 and self.CS.prev_cruise_buttons != 0:
                buttonEvents.append(create_button_event(
                    0, self.CS.prev_cruise_buttons, BUTTONS_DICT))
        ...
        if ret.cruiseState.available:
            ...
            self.toggle_gac(
                ret, self.CS, (self.CS.cruise_buttons[-1] == 3), 1, 3, 4, "-")
        else:
        ...
        ret, self.CS = self.get_sp_common_state(
            ret, self.CS, gap_button=(self.CS.cruise_buttons[-1] == 3))
        ...
        allow_enable = any(btn in ENABLE_BUTTONS for btn in self.CS.cruise_buttons) or any(
        ...
        events, ret = self.create_sp_events(
            self.CS, ret, events, main_enabled=True, allow_enable=allow_enable)
